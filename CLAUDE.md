...
| `add_payment_log.sql` | ⏳ Pending — **run to enable admin payment tracking** | Creates `payment_log` table (admin-only RLS policy gated to `dylashsav@gmail.com`). Tracks manual and Stripe-originated payments for both orgs and vault worker accounts. Required before running `add_admin_rpcs.sql`. |
| `add_admin_rpcs.sql` | ⏳ Pending — **run after add_payment_log.sql** | Creates 13 SECURITY DEFINER RPCs all hard-gated to `dylashsav@gmail.com`: `admin_get_stats`, `admin_get_orgs`, `admin_get_org_detail`, `admin_update_org`, `admin_get_workers`, `admin_get_worker_detail`, `admin_update_worker`, `admin_update_member`, `admin_move_member_org`, `admin_log_payment`, `admin_get_payment_log`, `admin_create_org`, `admin_suspend_org`. Powers `admin.html`. Must run `add_payment_log.sql` first. |
