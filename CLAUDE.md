...
| `worker_accounts` | **Worker Vault** — portable worker identity; `id (= auth.uid()), email, full_name, plan ('free'\|'vault'), plan_expires, stripe_customer_id, stripe_subscription_id`, plus worker-owned reminder prefs `reminder_enabled, reminder_days, reminder_missing` (default on/30/on). NOT org-scoped (worker-owned). `plan`/`plan_expires`/`stripe_*` are edge-function-owned AND column-UPDATE-revoked from `authenticated` (see `add_vault_reminders.sql`); only `full_name` + `reminder_*` are worker-updatable. |
| `vault_notification_log` | **Worker Vault** — throttle log for the `vault-reminders` edge function; `worker_account_id, doc_key, milestone ('advance'\|'expiry_day'\|'expired'\|'missing'), target_date, sent_at`. Worker-scoped read RLS; written only by the service-role edge function. One row per milestone sent so each fires once (a renewed doc with a new `target_date` re-triggers; `missing` throttled weekly). |
...
| `harden_get_worker_portal_org_id.sql` | ✅ Run (superseded by fix_worker_portal_id_lookup.sql — do NOT re-run, it would regress the function to the 2-param pre-switcher signature) | Added `RAISE EXCEPTION 'p_org_id is required'` guard to get_worker_portal + `SET search_path = public`. Subsequent migrations replaced this definition. |
| `fix_worker_portal_id_lookup.sql` | ✅ Run (2026-06-14, via MCP) | **Full live definition** of `get_worker_portal` as of 2026-06-14. Adds ID-only lookup branch for code-login workers with no email (`p_email='' AND p_worker_id IS NOT NULL`). Consolidates all prior changes: NULL guard, p_worker_id profile-pinning, competencies, project_files. Always use this file (not harden_ or switcher_ files) as the canonical definition. |
...
| `add_payment_log.sql` | ✅ Run (2026-06-13, via MCP) | Creates `payment_log` table (admin-only RLS policy gated to `dylashsav@gmail.com`). Tracks manual and Stripe-originated payments for both orgs and vault worker accounts. Required before running `add_admin_rpcs.sql`. |
| `add_admin_rpcs.sql` | ✅ Run (2026-06-13, via MCP) | Creates 13 SECURITY DEFINER RPCs all hard-gated to `dylashsav@gmail.com`: `admin_get_stats`, `admin_get_orgs`, `admin_get_org_detail`, `admin_update_org`, `admin_get_workers`, `admin_get_worker_detail`, `admin_update_worker`, `admin_update_member`, `admin_move_member_org`, `admin_log_payment`, `admin_get_payment_log`, `admin_create_org`, `admin_suspend_org`. Powers `admin.html`. `admin_update_org`/`admin_update_worker` also take Stripe ID params; `admin_create_org` inserts `settings (id text = org_id::text, org_id)`. |
| `add_leads.sql` | ✅ Run (2026-06-13, via MCP) | Lightweight sales CRM. Creates `leads` + `lead_notes` tables (admin-only RLS gated to `dylashsav@gmail.com`) + anon-callable `capture_lead()` SECURITY DEFINER RPC (dedupes by email) for the marketing site, plus admin RPCs: `admin_get_leads`, `admin_get_lead_detail`, `admin_update_lead`, `admin_add_lead_note`, `admin_create_lead`, `admin_delete_lead`. Powers the "Leads" tab in `admin.html`; captured from index.html Request-Trial form + a demo-gate modal. |
