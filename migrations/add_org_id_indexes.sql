...
--
-- Plain CREATE INDEX (not CONCURRENTLY) so the whole script runs in the
-- Supabase SQL Editor, which wraps statements in a single transaction
-- (CONCURRENTLY cannot run inside a transaction block). At current data
-- volumes each index builds in milliseconds and the brief lock is harmless.
-- If you ever re-run this on a large, busy table, build that index
-- separately with CREATE INDEX CONCURRENTLY outside a transaction instead.
-- =============================================================================

-- ── workers ──
CREATE INDEX IF NOT EXISTS idx_workers_org_id
  ON workers (org_id);
CREATE INDEX IF NOT EXISTS idx_workers_org_active
  ON workers (org_id, active);

-- ── worker_documents ──
CREATE INDEX IF NOT EXISTS idx_worker_documents_org_id
  ON worker_documents (org_id);
CREATE INDEX IF NOT EXISTS idx_worker_documents_org_worker
  ON worker_documents (org_id, worker_id, active);

-- ── worker_document_files ──
CREATE INDEX IF NOT EXISTS idx_worker_document_files_org_id
  ON worker_document_files (org_id);
CREATE INDEX IF NOT EXISTS idx_worker_document_files_org_worker
  ON worker_document_files (org_id, worker_id, active);

-- ── document_sets ──
CREATE INDEX IF NOT EXISTS idx_document_sets_org_id
  ON document_sets (org_id);

-- ── document_set_items ──
CREATE INDEX IF NOT EXISTS idx_document_set_items_org_id
...
CREATE INDEX IF NOT EXISTS idx_project_assignments_org_worker
  ON project_assignments (org_id, worker_id, active);

-- ── project_assignment_files ──
CREATE INDEX IF NOT EXISTS idx_project_assignment_files_org_id
...
CREATE INDEX IF NOT EXISTS idx_accommodation_assignments_org_id
  ON accommodation_assignments (org_id);
CREATE INDEX IF NOT EXISTS idx_accommodation_assignments_org_worker
  ON accommodation_assignments (org_id, worker_id, active);

-- ── vehicle_assignments ──
CREATE INDEX IF NOT EXISTS idx_vehicle_assignments_org_id
  ON vehicle_assignments (org_id);
CREATE INDEX IF NOT EXISTS idx_vehicle_assignments_org_worker
  ON vehicle_assignments (org_id, worker_id, active);

-- ── accommodation_charges ──
CREATE INDEX IF NOT EXISTS idx_accommodation_charges_org_id
  ON accommodation_charges (org_id);

-- ── vehicle_charges ──
CREATE INDEX IF NOT EXISTS idx_vehicle_charges_org_id
  ON vehicle_charges (org_id);

-- ── resource_events ──
CREATE INDEX IF NOT EXISTS idx_resource_events_org_id
  ON resource_events (org_id);

-- ── compliance_documents ──
CREATE INDEX IF NOT EXISTS idx_compliance_documents_org_id
  ON compliance_documents (org_id);

-- ── deleted_items ──
CREATE INDEX IF NOT EXISTS idx_deleted_items_org_id
