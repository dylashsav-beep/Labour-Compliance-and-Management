...
-- ── Helper: resolve the calling user's role from profiles ──────────────────
...
-- ── profiles ───────────────────────────────────────────────────────────────
-- Users can always read their own profile; admins read all
CREATE POLICY profiles_select ON profiles FOR SELECT USING (
  id = auth.uid() OR get_my_role() = 'admin'
);
-- Only admins can update any profile; users can update their own name
...
CREATE POLICY settings_select ON settings FOR SELECT USING (
  get_my_role() IN ('admin','planner','compliance','viewer')
);
CREATE POLICY settings_write ON settings FOR ALL USING (
  get_my_role() IN ('admin','compliance')
);

-- ── document_sets ─────────────────────────────────────────────────────────
CREATE POLICY doc_sets_select ON document_sets FOR SELECT USING (
  get_my_role() IN ('admin','planner','compliance','viewer')
);
CREATE POLICY doc_sets_write ON document_sets FOR ALL USING (
  get_my_role() IN ('admin','compliance')
);

-- ── document_set_items ────────────────────────────────────────────────────
CREATE POLICY doc_set_items_select ON document_set_items FOR SELECT USING (
  get_my_role() IN ('admin','planner','compliance','viewer')
);
CREATE POLICY doc_set_items_write ON document_set_items FOR ALL USING (
  get_my_role() IN ('admin','compliance')
);
...
CREATE POLICY workers_write ON workers FOR ALL USING (
  get_my_role() IN ('admin','compliance')
);

-- ── worker_documents ──────────────────────────────────────────────────────
CREATE POLICY worker_docs_select ON worker_documents FOR SELECT USING (
  get_my_role() IN ('admin','planner','compliance','viewer')
);
CREATE POLICY worker_docs_write ON worker_documents FOR ALL USING (
  get_my_role() IN ('admin','compliance')
);

-- ── worker_document_files ─────────────────────────────────────────────────
CREATE POLICY worker_doc_files_select ON worker_document_files FOR SELECT USING (
  get_my_role() IN ('admin','planner','compliance','viewer')
);
CREATE POLICY worker_doc_files_write ON worker_document_files FOR ALL USING (
  get_my_role() IN ('admin','compliance')
);
...
CREATE POLICY projects_write ON projects FOR ALL USING (
  get_my_role() IN ('admin','planner')
);

-- ── project_assignments ───────────────────────────────────────────────────
CREATE POLICY proj_assign_select ON project_assignments FOR SELECT USING (
  get_my_role() IN ('admin','planner','compliance','viewer')
);
CREATE POLICY proj_assign_write ON project_assignments FOR ALL USING (
  get_my_role() IN ('admin','planner')
);

-- ── project_assignment_files ──────────────────────────────────────────────
CREATE POLICY proj_assign_files_select ON project_assignment_files FOR SELECT USING (
  get_my_role() IN ('admin','planner','compliance','viewer')
);
CREATE POLICY proj_assign_files_write ON project_assignment_files FOR ALL USING (
  get_my_role() IN ('admin','planner')
);

-- ── roster_week_allocations ───────────────────────────────────────────────
CREATE POLICY roster_select ON roster_week_allocations FOR SELECT USING (
  get_my_role() IN ('admin','planner','compliance','viewer')
);
CREATE POLICY roster_write ON roster_week_allocations FOR ALL USING (
  get_my_role() IN ('admin','planner')
);
...
CREATE POLICY properties_write ON properties FOR ALL USING (
  get_my_role() IN ('admin','planner')
);
...
CREATE POLICY accom_assign_select ON accommodation_assignments FOR SELECT USING (
  get_my_role() IN ('admin','planner','compliance','viewer')
);
CREATE POLICY accom_assign_write ON accommodation_assignments FOR ALL USING (
  get_my_role() IN ('admin','planner')
);

-- ── vehicle_assignments ───────────────────────────────────────────────────
CREATE POLICY veh_assign_select ON vehicle_assignments FOR SELECT USING (
  get_my_role() IN ('admin','planner','compliance','viewer')
);
CREATE POLICY veh_assign_write ON vehicle_assignments FOR ALL USING (
  get_my_role() IN ('admin','planner')
);

-- ── deleted_items ─────────────────────────────────────────────────────────
CREATE POLICY deleted_items_select ON deleted_items FOR SELECT USING (
  get_my_role() IN ('admin','compliance')
);
CREATE POLICY deleted_items_write ON deleted_items FOR ALL USING (
  get_my_role() IN ('admin','compliance')
);
