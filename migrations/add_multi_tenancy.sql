...
UPDATE tools                          SET org_id = '00000000-0000-0000-0001-000000000001' WHERE org_id IS NULL;
UPDATE tool_assignments               SET org_id = '00000000-0000-0000-0001-000000000001' WHERE org_id IS NULL;
UPDATE tool_charges                   SET org_id = '00000000-0000-0000-0001-000000000001' WHERE org_id IS NULL;
UPDATE worker_resource_return_requests SET org_id = '00000000-0000-0000-0001-000000000001' WHERE org_id IS NULL;
...
-- ── worker_resource_return_requests ──
DO $$ BEGIN
...
END $$;
CREATE POLICY "org read return requests"    ON worker_resource_return_requests FOR SELECT TO authenticated USING (org_id = current_org_id());
CREATE POLICY "org write return requests"   ON worker_resource_return_requests FOR ALL    TO authenticated USING (org_id = current_org_id()) WITH CHECK (org_id = current_org_id());
CREATE POLICY "anon insert return requests" ON worker_resource_return_requests FOR INSERT TO anon         WITH CHECK (true);
...
-- get_worker_portal: add optional p_org_id parameter so the portal can scope by org
...
BEGIN
  ...
  RETURN json_build_object(
    ...
    'tools',          (SELECT COALESCE(json_agg(t ORDER BY t.name), '[]'::json) FROM tools t WHERE t.active = true AND t.org_id = v_worker.org_id),
...
-- submit_resource_return: add org_id parameter and store it
...
BEGIN
  ...
  -- Resolve org: prefer explicit param, fall back to worker's org
...
GRANT EXECUTE ON FUNCTION submit_resource_return(text, uuid, text, uuid, text, text, bigint, text, text, date, uuid)
